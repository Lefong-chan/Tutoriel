...
(function () {
    ...
    roomSyncTimer = setInterval(async function() {
      ...
      try {
        ...
        if (res.status === 'accepted') {
          ...
          // Le jeu a démarré → les deux joueurs redirigés vers fanorona.html
          clearInterval(roomSyncTimer); roomSyncTimer = null;
          if (window._stopRoomChat) window._stopRoomChat();
          var myCol = !isSender ? 'mena' : 'maintso'; // sender=maintso(Green), receiver=mena(Red)
  ...
  document.getElementById('gsLaunchBtn').addEventListener('click', function () {
    if (gameOrigin === 'room' && !matchupData) {
      ...
      // Vérifier que le receiver est ready
      var launchBtn = document.getElementById('gsLaunchBtn');
      ...
      callSessionApi('start-fanorona-game', {
        ...
        if (res.success) {
          // Arrêter le sync polling et le chat avant de quitter
          if (roomSyncTimer) { clearInterval(roomSyncTimer); roomSyncTimer = null; }
          if (window._stopRoomChat) window._stopRoomChat();
          // Rediriger vers fanorona.html avec les paramètres
          window.location.href = 'fanorona.html'
            + '?gameId=' + encodeURIComponent(res.gameId)
            + '&color='  + encodeURIComponent(res.myColor)
