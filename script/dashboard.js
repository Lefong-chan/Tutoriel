...
(function () {
  ...
  document.getElementById('ctxPlayBtn').addEventListener('click', function () {
    ...
    // Ne pas fermer le menu ni le modal friends — garder visible
    // Juste fermer le ctx menu mais laisser playersModal ouvert
    closeCtxMenu();
  ...
  async function startCtxInvite(uid, name) {
    ...
    ctxInviteUid = uid;
    ctxInviteId = null;
    // Mémoriser si on est dans friends au moment de l'invitation
    ...
    // Afficher countdown dans le bouton
    ...
    ctxInviteStatusTimer = setInterval(async function() {
      ...
      try {
        ...
        if (sRes.status === 'accepted') {
          ...
          if (countdown) { countdown.style.display = 'none'; countdown.textContent = ''; }
          // Fermer le modal friends avant d'ouvrir la room
