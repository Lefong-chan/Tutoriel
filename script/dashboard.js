...
(function () {
  ...
  // receiverReady supprimé — utiliser receiverIsReady (ligne ~879) comme source unique
      ...
      try {
        ...
        if (res.status === 'accepted') {
          if (isSender) {
            ...
            receiverIsReady = rReady;
            var launchBtn = document.getElementById('gsLaunchBtn');
            var notReadyEl = document.getElementById('gsOpponentNotReady');
            if (launchBtn) {
              launchBtn.innerHTML = '<i class="fas fa-play"></i> Start Game';
              launchBtn.disabled = !receiverIsReady;
              launchBtn.style.background = receiverIsReady ? '' : 'linear-gradient(to right, #94a3b8, #64748b)';
            }
            if (notReadyEl) notReadyEl.style.display = receiverIsReady ? 'none' : 'inline';
          } else {
          ...
          // Jeu démarré → rediriger vers fanorona.html
          clearInterval(roomSyncTimer); roomSyncTimer = null;
          if (window._stopRoomChat) window._stopRoomChat();
  ...
  document.getElementById('gsLaunchBtn').addEventListener('click', function () {
    if (gameOrigin === 'room' && !matchupData) {
      ...
      if (!matchupData || !matchupData.inviteId) return;
      var launchBtn = document.getElementById('gsLaunchBtn');
      if (launchBtn) launchBtn.disabled = true;
      callSessionApi('start-fanorona-game', {
        uid:      currentUser.uid,
  ...
  // ================================================================
  // ROOM CHAT — via /api/session (server-side RTDB)
  // ================================================================
  // Ny chat dia mandeha amin'ny /api/session toy ny actions hafa rehetra.
  // Tsy misy REST direct mankany amin'ny RTDB avy amin'ny browser —
  // ny server (session.js) no mitantana ny read/write RTDB.
  // ================================================================

  var GS_CHAT_MAX   = 30;   // 30 caractères max par message (emoji = 1)
  ...
  async function sendChatMessage() {
    if (!gsChatInput || !_chatRoomId || !currentUser) return;
    ...
    // Loading spinner sur le bouton send
    if (gsChatSendBtn) {
      gsChatSendBtn.classList.add('loading');
      gsChatSendBtn.disabled = true;
    }

    try {
      await callSessionApi('send-chat-message', {
        uid:      currentUser.uid,
        inviteId: _chatRoomId,
        text:     text
      });
      ...
      // Retirer le loading spinner
      if (gsChatSendBtn) {
        gsChatSendBtn.classList.remove('loading');
        gsChatSendBtn.disabled = false;
      }
    }
  }
