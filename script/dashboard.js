...
(function () {
  ...
  document.addEventListener('click', function (e) {
    ...
    // Garder le menu ouvert si une invitation est en cours
    if (ctxInviteTimer) return;
    closeCtxMenu();
  });
  ...
  var ctxInviteTimer = null;
  var ctxInviteStatusTimer = null;
  var ctxInviteId = null;
  var ctxInviteUid = null;
  var _ctxInviteFromFriends = false; // mémorise si invitation depuis friends

  document.getElementById('ctxPlayBtn').addEventListener('click', function () {
    if (ctxInviteTimer) return; // already counting
    var uid = activeFriendUid;
    var name = activeFriendName;
    // Pas de closeCtxMenu — le menu reste ouvert pendant le countdown
    startCtxInvite(uid, name);
  });

  function resetCtxPlayBtn() {
    var playBtn = document.getElementById('ctxPlayBtn');
    var countdown = document.getElementById('ctxPlayCountdown');
    if (playBtn) { playBtn.disabled = false; playBtn.style.opacity = ''; }
    if (countdown) { countdown.style.display = 'none'; countdown.textContent = ''; }
  }

  async function startCtxInvite(uid, name) {
    var playBtn = document.getElementById('ctxPlayBtn');
    var countdown = document.getElementById('ctxPlayCountdown');
    ctxInviteUid = uid;
    ctxInviteId = null;
    _ctxInviteFromFriends = document.getElementById('playersModal').classList.contains('active');

    // Envoyer invitation
    try {
      var res = await callSessionApi('send-game-invite', {
        uid: currentUser.uid, toUid: uid,
        game: selectedGame || 'fanorona', color: selectedColor || 'green', minutes: selectedMinutes || 5
      });
      ctxInviteId = res.inviteId;
    } catch(err) {
      showToast(err.message || 'Failed to send invitation.', 'error');
      return;
    }

    // Disable le bouton + opacity 0.8 + countdown visible
    if (playBtn) { playBtn.disabled = true; playBtn.style.opacity = '0.8'; }
    if (countdown) { countdown.style.display = 'inline'; countdown.textContent = '10'; }

    var count = 10;
    ctxInviteTimer = setInterval(function() {
      count--;
      if (countdown) countdown.textContent = count > 0 ? count : '0';
      if (count <= 0) {
        clearInterval(ctxInviteTimer); ctxInviteTimer = null;
        resetCtxPlayBtn();
        callSessionApi('cancel-game-invite', { uid: currentUser.uid, inviteId: ctxInviteId }).catch(function(){});
        if (ctxInviteStatusTimer) { clearInterval(ctxInviteStatusTimer); ctxInviteStatusTimer = null; }
        ctxInviteId = null; ctxInviteUid = null;
      }
    }, 1000);

    // Polling accepté
    ctxInviteStatusTimer = setInterval(async function() {
      ...
      try {
        ...
        if (sRes.status === 'accepted') {
          ...
          resetCtxPlayBtn();
          // Fermer ctx menu + playersModal avant d'ouvrir la room
          closeCtxMenu();
          closeModal(document.getElementById('playersModal'));
          selectedGame = sRes.game; selectedMinutes = sRes.minutes; selectedColor = sRes.color;
          gameOrigin = 'room';
          openGameSetup(sRes.game, {
            senderUsername: sRes.senderUsername,
            receiverUsername: sRes.receiverUsername,
            isSender: true,
            inviteId: ctxInviteId
          });
          ctxInviteId = null; ctxInviteUid = null;
          ...
          clearInterval(ctxInviteStatusTimer); ctxInviteStatusTimer = null;
          resetCtxPlayBtn();
          if (sRes.status === 'declined') showToast(name + ' declined your invitation.', 'info');
          ctxInviteId = null; ctxInviteUid = null;
        }
      } catch(e) {}
    }, 2000);
  ...
  function openGameSetup(game, matchup) {
    ...
    var gsMatchupWith = document.getElementById('gsMatchupWith');
    var gsMatchupPending = document.getElementById('gsMatchupPending');
    ...
    if (gameOrigin === 'room') {
      ...
      if (matchup) {
        // Olona roa — maseho Sender × Receiver
        if (gsMatchupWith) gsMatchupWith.style.display = 'flex';
        if (gsMatchupPending) gsMatchupPending.style.display = 'none';
        if (gsMatchupSender) gsMatchupSender.textContent = matchup.senderUsername;
        if (gsMatchupReceiver) gsMatchupReceiver.textContent = matchup.receiverUsername;
      } else {
        // Tsy misy adversaire — message centré fotsiny
        if (gsMatchupWith) gsMatchupWith.style.display = 'none';
        if (gsMatchupPending) gsMatchupPending.style.display = 'block';
      }
    } else {
    ...
    if (isRoom && !matchup) {
      ...
      // Arrêter le chat si on revient sans matchup
      ...
      // Démarrer le chat RTDB
      if (window._initRoomChat && matchup.inviteId) window._initRoomChat(matchup.inviteId);
    } else {
    ...
    roomSyncTimer = setInterval(async function() {
      ...
      try {
        ...
        if (res.status === 'accepted') {
          ...
          if (window._roomOfflineTimer) { clearTimeout(window._roomOfflineTimer); window._roomOfflineTimer = null; }
          if (!isSender) {
            // Receiver: sender a annulé → fermer room
            ...
            // Si le receiver était dans friends avant → rouvrir friends
            if (_receiverCameFromFriends) {
              _receiverCameFromFriends = false;
              var pm = document.getElementById('playersModal');
              if (pm) { _friendsFirstLoad = true; openModal(pm); loadFriendsPanel(); loadRequestsBadge(); loadSentBadge(); }
            }
          }
          matchupData = null;
        }
  ...
  async function quitRoom() {
    ...
    // Stopper sync polling + offline timer + chat
    if (roomSyncTimer) { clearInterval(roomSyncTimer); roomSyncTimer = null; }
    if (window._roomOfflineTimer) { clearTimeout(window._roomOfflineTimer); window._roomOfflineTimer = null; }
    if (window._stopRoomChat) window._stopRoomChat();
    // Fermer tout
    ...
    closeModal(gameSetupModal);
    closeModal(gameSelectModal);
    var _wasFromFriends = _ctxInviteFromFriends;
    matchupData = null;
    _ctxInviteFromFriends = false;
    // Si le sender avait invité depuis friends → rouvrir friends
    if (_wasFromFriends && roomIsSender) {
      var pm3 = document.getElementById('playersModal');
      if (pm3) { _friendsFirstLoad = true; openModal(pm3); loadFriendsPanel(); loadRequestsBadge(); loadSentBadge(); }
    }
  }
  ...
  var _receiverCameFromFriends = false;
  ...
  ginAcceptBtn.addEventListener('click', async function() {
    ...
    hideGameInviteNotif();
    // Mémoriser si on était dans le modal friends
    _receiverCameFromFriends = document.getElementById('playersModal').classList.contains('active');
  ...
  // ================================================================
  // ROOM CHAT — Realtime Database (RTDB)
  // ================================================================
  // Règles:
  //  - max 30 caractères par message (emoji = 1)
  //  - max 20 messages affichés; les anciens sont supprimés auto dans RTDB
  //  - messages envoyés → droite (bulle violette)
  //  - messages reçus  → gauche (bulle grise)
  //  - scroll vertical si débordement
  //  - "No messages yet…" disparaît dès le premier message
  // ================================================================

  var RTDB_BASE_URL = (typeof FIREBASE_DATABASE_URL !== 'undefined')
    ? FIREBASE_DATABASE_URL
    : null; // sera défini via window.FIREBASE_DATABASE_URL injecté dans le HTML

  var GS_CHAT_MAX    = 30;   // ← 30 caractères max (emoji inclus)
  var GS_CHAT_LIMIT  = 20;   // ← 20 messages visibles max
  ...
  // Référence RTDB courante du chat (nulle si pas en room)
  var _chatRoomId     = null;   // inviteId utilisé comme clé de chat
  var _chatListener   = null;   // fonction de callback SSE/polling

  // ── Utilitaires RTDB (REST) ─────────────────────────────────────
  // On utilise la REST API Firebase RTDB pour éviter d'inclure le SDK client.
  // FIREBASE_DATABASE_URL doit être défini dans dashboard.html :
  //   <script>window.FIREBASE_DATABASE_URL = "https://xxx.firebaseio.com";</script>

  function getRtdbUrl() {
    return window.FIREBASE_DATABASE_URL || '';
  }

  async function rtdbPut(path, data) {
    var url = getRtdbUrl() + path + '.json';
  ...
  // ── Comptage correct (emoji = 1) ───────────────────────────────
  function chatCharCount(val) { return Array.from(val).length; }

  // ── Initialiser le chat d'une room ─────────────────────────────
  function initRoomChat(inviteId) {
    _chatRoomId = inviteId;
    // Vider le chat
    if (gsChatMessages) {
      gsChatMessages.innerHTML = '<span class="gs-chat-placeholder">No messages yet\u2026</span>';
    }
    // Démarrer l'écoute des messages
    startChatPolling(inviteId);
  }
  ...
  // ── Polling des messages (toutes les 1.5s) ─────────────────────
  var _chatPollTimer    = null;
  var _chatLastMsgCount = 0;

  function startChatPolling(inviteId) {
    if (_chatPollTimer) { clearInterval(_chatPollTimer); _chatPollTimer = null; }
    _chatLastMsgCount = 0;
    _chatPollTimer = setInterval(async function() {
      if (!_chatRoomId || _chatRoomId !== inviteId) {
        clearInterval(_chatPollTimer); _chatPollTimer = null; return;
      }
      try {
        var data = await rtdbGet('/roomChats/' + inviteId + '/messages');
        if (!data) return;
        var msgs = [];
        Object.keys(data).forEach(function(k) { msgs.push(data[k]); });
        msgs.sort(function(a, b) { return (a.ts || 0) - (b.ts || 0); });
        // Seulement les 20 derniers
        if (msgs.length > GS_CHAT_LIMIT) msgs = msgs.slice(msgs.length - GS_CHAT_LIMIT);
  ...
  function renderChatMessages(msgs) {
    if (!gsChatMessages) return;
    if (!msgs || msgs.length === 0) {
      gsChatMessages.innerHTML = '<span class="gs-chat-placeholder">No messages yet\u2026</span>';
      return;
    }
    gsChatMessages.innerHTML = '';
    var myUid = currentUser ? currentUser.uid : null;
    msgs.forEach(function(msg) {
      var isMine = msg.senderUid === myUid;
      var wrap = document.createElement('div');
      wrap.className = 'gs-msg-wrap ' + (isMine ? 'gs-msg-mine' : 'gs-msg-theirs');
      var bubble = document.createElement('div');
      bubble.className = 'gs-msg-bubble';
      bubble.textContent = msg.text;
      wrap.appendChild(bubble);
      gsChatMessages.appendChild(wrap);
    });
    // Scroll vers le bas
    gsChatMessages.scrollTop = gsChatMessages.scrollHeight;
  }

  // ── Envoyer un message ─────────────────────────────────────────
  async function sendChatMessage() {
    if (!gsChatInput || !_chatRoomId) return;
    var text = gsChatInput.value.trim();
    if (!text || chatCharCount(text) === 0) return;
    // Limiter à 30 caractères
    var chars = Array.from(text);
    if (chars.length > GS_CHAT_MAX) text = chars.slice(0, GS_CHAT_MAX).join('');

    gsChatInput.value = '';
    gsChatInput.style.height = 'auto';
    if (!currentUser) return;

    var msg = {
      senderUid:      currentUser.uid,
      senderUsername: currentUser.username || currentUser.uid,
      text:           text,
      ts:             Date.now()
    };

    try {
      var path = '/roomChats/' + _chatRoomId + '/messages';
      // 1. Envoyer le message
      var result = await rtdbPost(path, msg);
      var newKey = result && result.name;

      // 2. Vérifier et limiter à 20 messages max (supprimer les anciens)
      if (newKey) {
        pruneOldMessages(_chatRoomId);
      }
    } catch(e) {}
  }

  // ── Supprimer automatiquement les messages > 20 ────────────────
  async function pruneOldMessages(inviteId) {
    try {
      var data = await rtdbGet('/roomChats/' + inviteId + '/messages');
      if (!data) return;
      var keys = Object.keys(data);
      if (keys.length <= GS_CHAT_LIMIT) return;
      // Trier par timestamp
      var sorted = keys.map(function(k) { return { key: k, ts: data[k].ts || 0 }; });
      sorted.sort(function(a, b) { return a.ts - b.ts; });
      // Supprimer les plus anciens
      var toDelete = sorted.slice(0, sorted.length - GS_CHAT_LIMIT);
      for (var i = 0; i < toDelete.length; i++) {
        await rtdbDelete('/roomChats/' + inviteId + '/messages/' + toDelete[i].key);
      }
    } catch(e) {}
  }

  // ── Input: resize auto + limite 30 chars ──────────────────────
  if (gsChatInput) {
    gsChatInput.addEventListener('input', function() {
      // Limiter à 30 caractères
      if (chatCharCount(this.value) > GS_CHAT_MAX) {
        var chars = Array.from(this.value);
        this.value = chars.slice(0, GS_CHAT_MAX).join('');
      }
      // Auto-resize: max 2 lignes
      this.style.height = 'auto';
      var lineH = parseFloat(getComputedStyle(this).lineHeight) || 16;
      var pad   = parseFloat(getComputedStyle(this).paddingTop) + parseFloat(getComputedStyle(this).paddingBottom);
      var maxH  = Math.round(lineH * 2 + pad);
    ...
    gsChatInput.addEventListener('keydown', function(e) {
      ...
      var allowed = ['Backspace','Delete','ArrowLeft','ArrowRight','ArrowUp','ArrowDown','Home','End'];
      if (chatCharCount(this.value) >= GS_CHAT_MAX && !allowed.includes(e.key) && !e.ctrlKey && !e.metaKey) {
        if (this.selectionStart === this.selectionEnd) e.preventDefault();
      }
    });
  }

  if (gsChatSendBtn) {
    gsChatSendBtn.addEventListener('click', function() { sendChatMessage(); });
  }

  // ── Exposer initRoomChat / stopRoomChat pour openGameSetup ─────
  window._initRoomChat = initRoomChat;
  window._stopRoomChat = stopRoomChat;
