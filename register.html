...
/* Remove footer (support + timer) if inherited */
.error-message .blocked-footer {
  display: none;
}
...
<div id="globalErrorMessage" class="blocked-message error-message">
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Allow only numbers + disable Verify if OTP < 6
  document.getElementById("otpInput")
  ...
  ?.addEventListener("submit", async e => {
    ...
    if (registerMode === "email") {
      ...
      if (!rawValue.includes("@"))
        return showGlobalError("Invalid email.", "left");
      
      const emailLower = rawValue.toLowerCase();
      
      bodyData = { email: emailLower, password };
      identifierForOTP = emailLower;
      
    } else {
      
      if (rawValue.length < 8)
        return showGlobalError("Invalid phone.", "left");
      
      bodyData = { phone: rawValue, password };
    }
    ...
    try {
      ...
      if (data.phoneVerificationRequired && rawValue) {
        openPhoneOtpModal(rawValue);
        return;
      }
...
function openOtpModal(identifier) {
  ...
  // Disable background scroll (premium feel)
...
async function verifyPhoneOTP() {
  ...
  try {
    ...
    const res = await fetch("/api/verify-otp-phone", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
      body: JSON.stringify({
...
async function resendPhoneOTP() {
  ...
  try {
    ...
    const res = await fetch("/api/resend-otp-phone", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
      body: JSON.stringify({
...
function startLoginBlockCountdown(seconds) {
  ...
  update();
  blockedInterval = setInterval(update, 60000);
}
...
function showGlobalError(message, side = "right") {
  ...
  globalErrorMessage.classList.remove("left", "right");
